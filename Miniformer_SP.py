...
for fold, (train_indices, test_indices) in enumerate(kfold.split(full_dataset)):
    ...
    for epoch in range(epochs):
        ...
        for i, data in enumerate(tqdm(train_loader,ncols=80,desc=f'Training | Fold:{fold + 1},Epoch:{epoch+1}')):
            ...
            s_v0 = torch.norm(_p0, p=1)
            s_v1 = torch.norm(_p1, p=1)
            ...
            batch_loss = batch_loss+ miu*(l2_0 + l2_1) +lamda*(d_v0 + d_v1) +gamma*(s_v0 + s_v1)
        ...
        if epoch == epochs - 1:
            plt.plot(np.arange(175),np.squeeze(_p1.detach().cpu().numpy()))
